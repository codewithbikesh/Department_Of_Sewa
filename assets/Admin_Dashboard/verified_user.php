...
if (isset($_GET['verifiedFlexSwitchCheckDefault'])) {
    $flexSwitchCheckDefault = $_GET['verifiedFlexSwitchCheckDefault'];
    $usernameValue = $_GET['usernameValue'];
    // Connect to the database
    $conn = connectMyDB();

    // Check if the active_status is 0
    $sql = "SELECT verified FROM login_det WHERE user_email='$usernameValue'";
    ...
    if ( $row['verified']== 0) {
        ...
        $updateSql = "UPDATE login_det SET  verified = 1 WHERE user_email='$usernameValue'";
        ...
        $updateSql = "UPDATE login_det SET verified = 0 WHERE user_email='$usernameValue'";
        $updateStmt = mysqli_prepare($conn, $updateSql);
        mysqli_stmt_execute($updateStmt);
        ...
        $response = array('result' => false, 'message' => 'No rows with verified = 0 found');
    }
