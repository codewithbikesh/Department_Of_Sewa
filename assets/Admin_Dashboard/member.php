...
?>
<div class="topbar pt-3" style="background:#424a4a;"><a class="btn"><i class="bi bi-list p-3 text-white" id="colpsCustom"></i></a><span class="text-white fw-bold mt-3">Member</span>
</div>
...
<nav class="navigationbar" style="background-color:#53efd9; style="overflow-x:hidden;"">
      ...
      <a class="dropdown-toggle fs-10 btn" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false" style="color:white; text-decoration:none; background-color:#033b40;">Select Filter Option</a>
       ...
       <button class="btn" style="background-color:#033b40; color:white;" id="sendEmailBtn">Send Event SMS</button>
       <button class="btn" style="background-color:#033b40; color:white;" id="sendNoticeSMS">Send Notice SMS</button>
  ...
  <li><a class="dropdown-item">Region&raquo;</a>
       <ul class="dropdown-menu dropdown-submenu submenu2">
       <?php
         $selectQuery2="SELECT DISTINCT `Region` FROM `center_list` order by `Region`";
         $con = connectMyDB();
         $queryPass2=mysqli_query($con,$selectQuery2);
         while( $result = mysqli_fetch_assoc($queryPass2)){
          ?>
        <li><a class="dropdown-item showing" id="byEducation" onclick="GetReport('Region','<?php echo $result['Region']?>');"><?php echo $result["Region"] ?></a> </li>
        <?php
         }
        ?>
       </ul>
  ...
  <li><a class="dropdown-item">Occupation&raquo;</a>
       <ul class="dropdown-menu dropdown-submenu submenu2">
       <?php
         $selectQuery2="SELECT DISTINCT `rProfession` FROM `introductiondetails` order by `rProfession`";
         $con = connectMyDB();
         $queryPass2=mysqli_query($con,$selectQuery2);
         while( $result = mysqli_fetch_assoc($queryPass2)){
          ?>
        <li><a class="dropdown-item showing" id="byEducation" onclick="GetReport('occupation','<?php echo $result['rProfession']?>');"><?php echo $result["rProfession"] ?></a> </li>
        <?php
         }
        ?>
       </ul>
   ...
   <tbody class="showByFilter" id="tbody_1">
      ...
      <tr>
          <td style="width:100px; padding-top:20px; display:flex; justify-content:space-evenly;">
             ...
             <a href="memberDelete.php?id=<?php echo $result['id'] ?>"><i class="fa fa-trash" aria-hidden="true" style="color:black; font-size:18px;"></i></a>
         </td>
         <td><img src="image_upload/profilPicture/<?php $img1 = $result['image']; echo $img1; ?>" onerror="this.src='user_icon.png'" class="card images-img-top image" id="backup_picture" alt="" style="height:2rem; width:3rem;"></td>
...
//  Start send notice sms code line from here 
//  Start send notice sms code line from here 
$("#sendNoticeSMS").click(function() {
     alert("you clicked me");
      var selectedData = [];
      let onlyMobile = $("#onlyMobile").text().trim();
      $(".emailChecked:checked").each(function() {
